...
Initially I had MovieActor relations and MovieDirector relations entered with the mid and aid/did respectively. However, after speaking to a TA and thinking about this for a little, this seemed impractical, so I changed it to to a drop-down select menu for the values. Obviously, this is not very practical as the database gets larger and larger. For now, we see a lag when the menu is opened and the initial load on the page, but a TA confirmed that for this project it is okay to deal with this. 